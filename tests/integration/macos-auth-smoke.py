...
compiler and /usr/bin/openssl for DES. Nothing is typed into the app; displayed
UI is not asserted.
"""
import argparse
...
def run_case(app, case, work):
    ...
    env = {} if case == 'vanished' else {'VNC_PASSWORD': PASSWORD if case == 'correct' else 'wrong-pw'}
    state = work / case
    launched = isolated.launch(app, state, ['-SecurityTypes=VncAuth', '-SendClipboard=0', '-AcceptClipboard=0',
...
def main():
    ...
    parser.add_argument('--case', action='append', choices=['correct', 'rejected', 'vanished'])
    args = parser.parse_args()
    cases = args.case or ['correct', 'rejected', 'vanished']
