...
opens TidyVNC › Settings…, clears "Receive clipboard from server", presses
Apply and quits. Launch 2: the same isolated copy and state are started again
with no arguments. The Recent connections popover must list the endpoint, and
Settings must show the cleared default with an app-default source.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='tidyvnc-persistence-') as temporary:
        ...
        try:
            ...
            quit_app(tool, first)
            print(f'PASS launch 1: connected to {peer.endpoint}, cleared the receive-clipboard default, applied and quit')
