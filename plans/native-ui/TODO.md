...
- [ ] N0.5 Capture matched performance workloads and budgets: idle/scrolling/1080p/4K/multi-view, p50/p95 latency, CPU, memory, copies and damage. Record existing scaling budget requirements and provisional 10% regression threshold.
  ...
  - [x] Matched FLTK presentation latency without source changes: an injected
...
- [ ] N5.9 Run matched FLTK/native benchmarks against N0 budgets; record latency/CPU/memory/copy/damage results and resolve or explicitly review regressions before cutover.
  - Partial (2026-09-23): matched CPU/RSS/throughput per PERFORMANCE.md shows no
    regression on those metrics. Matched presentation latency (FLTK via injected
    draw trace) shows no regression: full-frame p50 about 15 ms native vs 60 ms
    FLTK. Allocation rate, other machines and the budget review remain.
...
- [x] N6.11 Validate rollback to retained FLTK artifact using untouched legacy data and explicit profile export; do not overwrite native stores or transfer credentials automatically. 2026-09-23: `tests/integration/macos-rollback-smoke.py <TidyVNC.app> <vncviewer>` seeds an isolated state with the FLTK viewer's `default.tidyvnc` (AutoSelect=0, PreferredEncoding=ZRLE), its server history and an upstream `default.tigervnc`.
  1. The native app connects to `native-security-peer`, writes only its own history store, and leaves all three legacy files byte-identical.
  2. The retained Release FLTK viewer (`build/hidpi-release`) then runs with the same HOME/XDG roots. It connects with ZRLE as its first real encoding, so it read its untouched defaults, and it leaves the native store unchanged.
